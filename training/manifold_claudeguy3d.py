...
# The full 14-face cast: neutral + the 13 sheet moods. Parametric versions
# (compose with blink/gaze) where they exist; sheet painters (props: hat,
# monocle, mushroom cloud, groucho kit, mouth-tentacles) for the rest.
FACES = [
    ("param", "neutral"), ("sheet", "wideeyed"), ("param", "serene"),
    ("param", "wince"), ("param", "angry"), ("sheet", "mindblown"),
    ("sheet", "monocle"), ("param", "pleading"), ("sheet", "suspicious"),
    ("sheet", "cowboy"), ("param", "melancholy"), ("sheet", "disguise"),
    ("param", "dizzy"), ("sheet", "modecollapse"),
]
def _face_z(name):
    return [i for i, (_, n) in enumerate(FACES) if n == name][0] / (len(FACES) - 1)
...
def _A(droop, flut, splay, wig, dbl, spin, blnk, lky, lkx, face):
    return [droop, flut, splay, wig, dbl, spin, blnk, lky, lkx, _face_z(face)]

ANCHORS = {
    #                  droop flut  splay wig   dbl spin blnk lky   lkx   face
    "idle":         _A(0.20, 0.35, 0.50, 0.15, 0,  0,  0.6, 0.45, 0.50, "neutral"),
    "curious":      _A(0.05, 0.40, 0.60, 0.25, 0,  0,  0.4, 0.75, 0.75, "neutral"),
    "wideeyed":     _A(0.00, 0.55, 0.70, 0.30, 0,  0,  0.0, 0.55, 0.50, "wideeyed"),
    "serene":       _A(0.15, 0.30, 0.55, 0.10, 0,  0,  0.5, 0.45, 0.50, "serene"),
    "delight":      _A(0.05, 0.90, 0.65, 0.85, 1,  0,  0.3, 0.55, 0.50, "serene"),
    "wince":        _A(0.35, 0.45, 0.40, 0.35, 0,  0,  0.6, 0.45, 0.50, "wince"),
    "angry":        _A(0.10, 0.70, 0.60, 0.55, 1,  0,  0.1, 0.35, 0.50, "angry"),
    "mindblown":    _A(0.00, 1.00, 0.80, 0.95, 1,  0,  0.0, 0.50, 0.50, "mindblown"),
    "monocle":      _A(0.10, 0.15, 0.65, 0.05, 0,  0,  0.4, 0.45, 0.50, "monocle"),
    "pleading":     _A(0.45, 0.25, 0.45, 0.10, 0,  0,  0.1, 0.85, 0.50, "pleading"),
    "suspicious":   _A(0.25, 0.20, 0.40, 0.10, 0,  0,  0.5, 0.40, 0.25, "suspicious"),
    "cowboy":       _A(0.05, 0.65, 0.60, 0.70, 0,  1,  0.3, 0.50, 0.50, "cowboy"),
    "melancholy":   _A(0.95, 0.10, 0.30, 0.05, 0,  0,  0.2, 0.30, 0.50, "melancholy"),
    "disguise":     _A(0.15, 0.15, 0.45, 0.05, 0,  0,  0.7, 0.45, 0.50, "disguise"),
    "dizzy":        _A(0.50, 0.55, 0.50, 0.70, 1,  1,  0.2, 0.45, 0.50, "dizzy"),
    "modecollapse": _A(0.70, 0.50, 0.35, 0.60, 1,  0,  0.4, 0.45, 0.50, "modecollapse"),
}
...
def draw_mood(phase, z):
    ...
    kind, name = FACES[min(len(FACES) - 1, int(round(p["expr"] * (len(FACES) - 1))))]
    return draw_claudeguy(
        phase=phase, blink=blink, look=(p["look_x"], p["look_y"]),
        petal_flex=np.clip(flex, -1, 1),
        expression=name if kind == "param" else "neutral",
        sheet=name if kind == "sheet" else None,
