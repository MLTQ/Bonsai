...
Gate 3 compares `fused_nca_rollout` with already-gated chained steps, including a 3D per-step condition sequence and FiLM. Outputs and every gradient must agree within 2e-6 relative error.
...
|---|---|---|
| CI-by-hand before any fused training run | exit 0, "ALL GATES PASS" across step forward/backward and rollout equivalence | any fused_step.py or trainer-math change |
