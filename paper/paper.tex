...
oscillator rather than keeping time themselves. Two fine-tuning experiments
that remove the clock give a characterized partial result. Na\"{\i}vely
deleting the phase inputs collapses the gait to its \emph{temporal mean}
(a motionless blur of all phases) --- the cheapest MSE response to a schedule
the automaton cannot stay synchronized with. Adding a synchronization
curriculum (rollout horizons growing $12 \to 96$ steps) and \emph{oscillator
distillation} (an auxiliary loss asking the alive-population mean of two
hidden channels to encode $\cos\theta, \sin\theta$) prevents collapse: the
clockless organism then moves vigorously and indefinitely with intact
identity (consecutive-frame motion $6\times$ the collapsed baseline). But
recurrence analysis over 600-step rollouts shows no stable period --- only a
weak local minimum at the trained period --- indicating the distributed phase
code decoheres, plausibly because stochastic per-cell firing injects phase
noise faster than the learned coupling re-synchronizes it. The automaton
retains \emph{that} it moves, not \emph{the} cycle.

The resolution came from inverting the question (suggested by the first
author): rather than teaching cells to keep time, make stillness impossible.
\textbf{Heteroclinic cycling}: two pose targets under one conditioning flag,
with the loss always directed at the pole \emph{opposite} the nearest —
each pose trained unstable toward the other, in the spirit of winnerless
competition \cite{rabinovich2008transient}. The result, on a
diffusion-generated test creature with two lean poses: sustained, regular
clockless oscillation (pole-occupancy reads
\texttt{...LLLLLLLLRRRRRRRR...}), period $\approx 80$--$100$ automaton
steps — a number appearing nowhere in code or conditioning, emerging from
transit dynamics (roughly $2\times$ the training horizon). Phase-averaging
collapse is structurally prevented: the cycle's mean is precisely where
gradients point away from. The construction generalizes to directed pose
graphs — waypointed gaits, wandering idle constellations, and hysteresis
loops (non-palindromic directed cycles), making "never just an image" a
trainable property. MSE training softens precise
...
\bibitem{rabinovich2008transient}
M.~Rabinovich, R.~Huerta, P.~Varona, V.~Afraimovich.
