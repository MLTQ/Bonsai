...
### `condProvider` / `renderStyle` / `condCount`
- **Does**: Per-step conditioning values (phase, behavior flag) fed into uniforms; render style flag; the cond width the shader was compiled with
- **Rationale**: The shader is generated with `cond` baked in (`ncaMetalSource(cond:)`), so weights with a different cond width need a fresh NCASimulation, not just a buffer swap
